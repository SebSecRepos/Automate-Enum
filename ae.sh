...
EOF
    ...
    >&2
    echo -e "\t ${green}ae ${purple}axfr ${end}\t\t\t\t${yellow}Extract ${green}success AXFR Attack results${yellow} saved in ${purple} ./DNS_ENUM/ ${end}\n"
    >&2
    echo -e "\t ${green}ae ${purple}mdata ${end}[-u url_list]\t\t\t${yellow} Extract and filter metadata from files in urls ${end}\n"
    >&2
    ...
    >&2
    echo -e "\t ${green}ae ${purple}ht ${end}[-d domain_list]\t\t\t${yellow}Url analysis using httpx ${end}\n"
    >&2
    echo -e "\t ${green}ae ${purple}cr ${end}[-l url_list]\t\t\t${yellow}Filter for ${red}interesting files & routes${yellow} in a url list. ${green}Useful with ${purple}ur${green} module results${end}\n"
    >&2
    echo -e "\t ------------------------------------------------------------------------------------ \n"
    >&2
    ...
    >&2
    echo -e "\t${purple}mdata ${yellow} ./metadata/${end}\n"
    >&2
...
function dm() {
    shift
    /opt/automate-enum/modules/1.Reccon/domain.sh $@
}
function ht() {
    shift
    /opt/automate-enum/modules/1.Reccon/httpx.sh $@
}
function sc() {
    shift
    /opt/automate-enum/modules/1.Reccon/scraping.sh $@
}
function ur() {
    shift
    /opt/automate-enum/modules/1.Reccon/ur.sh $@
}
function dnsa() {
    shift
    /opt/automate-enum/modules/1.Reccon/dnsa.sh $@
}
function axfr() {
    /opt/automate-enum/modules/1.Reccon/axfr_filter.sh
}
function mdata() {
    shift
    /opt/automate-enum/modules/1.Reccon/mdata.sh $@
}
function critical() {
    shift
    /opt/automate-enum/modules/1.Reccon/critical.sh $@
}
